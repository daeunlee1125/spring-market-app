document.addEventListener("DOMContentLoaded", function() {
    ...
    prodPrice.addEventListener('input', function() {
        const price = parseInt(this.value) || 0;  // 값이 없으면 0
        const point = Math.floor(price * 0.01);   // 1% 계산 (소수점 버림)
    ...
    form.addEventListener('submit', function(e) {
        ...
        // 6. 상품금액 체크
        const prodPrice = document.querySelector('[name="prod_price"]').value;
        ...
        // 8. 포인트 체크
        const prodPoint = document.querySelector('[name="prod_point"]').value;
        if(!prodPoint || prodPoint < 0) {
